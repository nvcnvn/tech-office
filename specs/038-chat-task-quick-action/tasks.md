...
- [X] T088 [P] Run `make test-mobile-one F=chat-task-capture` on both an Android and an iOS device, per the mobile testing rule. Passing end to end on a physical Galaxy S21 (Android 15) and on an iPhone SE (3rd gen) simulator. The physical iPhone could not be driven: Maestro's connected-iPhone driver fails to build in both 2.8.0 and 2.10.0 — its extracted `MaestroDriverLib` module ships empty — and 2.3.0's `test` does not enumerate physical iPhones at all. Running it exposed four product defects and several flow defects, all fixed; see the Android/iOS notes in the feature's change set.
- [ ] T089 Walk through `specs/038-chat-task-quick-action/quickstart.md` §5 manually, confirming SC-002 (a second conversion in a remembered channel takes exactly two interactions) and that a user without project access sees no chip. **Partly done**: steps 1-5 were walked on a physical Android device and an iOS simulator during T088, with screenshots reviewed at every step; steps 6-8 (lazy comment thread on first open, archived destination project, and a viewer without project access) were not exercised by hand. Both remaining properties have automated coverage: SC-002 by the "second conversion pre-fills the project collapsed" web E2E, and the invisible chip by `t.Run("ListTasksBySourceMessages omits the link entirely")`.

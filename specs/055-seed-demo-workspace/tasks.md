...
- [X] T001 Bring up the local stack with `make dev-up` and `make migrate` from the repo root, per [quickstart.md](quickstart.md) — Postgres, Redis and MinIO from `backend/docker-compose.yml`
- [X] T002 Establish the baseline: run `cd backend && go test ./integration/... -run TestDemoSeed -v` and record that it passes today, so a later failure is attributable to this change set
- [X] T003 [P] Run `cd backend && go run ./cmd seed-demo-org --subdomain demo` once against the local database and capture the current stdout block order, which the FR-021 assertion in T018 extends rather than replaces
...
- [X] T004 Add `--spare-password` flag to `SeedDemoOrgCommand` in `backend/cmd/seed_demo.go`, defaulting to the value of `--owner-password` when unset, per [contracts/seed-demo-org-cli.md](contracts/seed-demo-org-cli.md)
- [X] T005 Extend `demoSeedResult` in `backend/cmd/seed_demo.go` with `SpareEmail` and `SparePassword`, and add the `demoSpareGivenName = "Ben"` / `demoSpareFamilyName = "Keeler"` / `demoOpsProjectKey = "OPS"` constants beside the existing `demo*` block
- [X] T006 Thread a single `seededAt time.Time` captured once at the top of `seedDemoOrg` through every helper in `backend/cmd/seed_demo.go`, so all seeded dates derive from one instant relative to the run (FR-020) rather than from repeated `time.Now()` calls that could straddle midnight
- [X] T007 [P] Add `requireProjectState(ctx, pool, orgID, projectID dbuuid.UUID, projectKey, category, stateType string) (dbuuid.UUID, error)` to `backend/cmd/seed_demo.go`, looking up `collaboration.project_state` by `(organization_id, project_id, category, state_type)` and returning `fmt.Errorf("demo project %s has no %s state in category %q; reseed the project", ...)` on no rows (FR-018, research R10) — never by name or position, because `project_state` is unique on `(organization_id, project_id, position)` only
...
- [X] T010 [P] [US1] Create `backend/integration/demo_seed_deletion_test.go` on the `testWorld` pattern with `TestDemoSeedAccountDeletion`, seeding the demo workspace by invoking the command and obtaining a Connect client for the spare owner — structure per [contracts/test-scenarios.md](contracts/test-scenarios.md)
...
- [X] T025 [US2] In `backend/cmd/seed_demo.go`, resolve the two `GEN` states through `requireProjectState`: `category = 'todo'` and `category = 'in_progress'`, both `state_type = 'standard'` (FR-009, FR-018)
- [X] T026 [US2] Add `seedDemoWorkItems(ctx, pool, orgID, genProjectID, levelID, ownerID, spareID, workerID dbuuid.UUID, todoStateID, inProgressStateID dbuuid.UUID, seededAt time.Time) error` to `backend/cmd/seed_demo.go`, writing the six `collaboration.task` rows of §3 of [data-model.md](data-model.md) with `task_kind = 'standard'`, `identifier` `GEN-1`…`GEN-6`, `depth = 0`, `path = '{}'`, `reporter_employee_id` = primary owner, and the exact title / state / assignee / `due_date` distribution given there (FR-009 – FR-012)
- [X] T027 [US2] In `seedDemoWorkItems` in `backend/cmd/seed_demo.go`, write one `collaboration.task_assignee` row per item with `role = 'assignee'` and `assigned_by_employee_id` = primary owner, then set `collaboration.project.task_count = 6` and `next_task_number = 7` on `GEN` (FR-010, research R9)
...
- [X] T035 [P] [US3] In `backend/integration/demo_seed_test.go`, add `t.Run("when the background sweeps run against the seeded workspace")` with: the late instance is still `overdue` rather than written off as `missed` (FR-015), and the generation sweep creates no further instances (FR-017) — driving the reconciliation and generation logic directly rather than waiting on the scheduler

### Implementation for User Story 3

- [X] T036 [US3] Wire `refreshDemoProjects` (T009) into `seedDemoOrg` in `backend/cmd/seed_demo.go` so it runs against both `GEN` and `OPS` before any task or evidence requirement is written on every run, and confirm the counter resets leave `identifier` generation starting from 1 (FR-019, research R9)
- [X] T037 [US3] Audit every date written by `backend/cmd/seed_demo.go` and confirm each derives from the `seededAt` instant of T006 rather than from `now()` in SQL or a hardcoded date, so a re-run before a resubmission produces content that reads as current (FR-020)
...
- [X] T040 Run `cd backend && go test ./integration/...` — the whole suite must pass, not just `TestDemoSeed`
- [X] T041 [P] Run `make check-store-manifest` and confirm it exits `0` after the `docs/compliance/reviewer-notes.md` rewrite; the edit adds a credential and an instruction and must not introduce a capability claim outside `### Not requested` (research R12)
- [X] T042 [P] Perform the FR-018 failure injection from [quickstart.md](quickstart.md) on a scratch edit that is never committed: delete the `Overdue` state from the demo `OPS` project, re-run the seed, and confirm it exits non-zero naming the project and the missing category
- [X] T043 Perform the manual mobile pass of [quickstart.md](quickstart.md) step 5 on **both** an Android emulator and an iOS simulator, signing in as each of the three credentials and confirming zero of the four tabs shows a whole-screen empty state (SC-003, SC-004) — Android is not optional here, since the habitual test device is an iPhone SE and a narrow-Android regression would otherwise go unnoticed
- [X] T044 Perform the manual deletion pass of [quickstart.md](quickstart.md) step 4 on a device: delete the spare account, confirm you are signed out and cannot sign in again, then confirm the primary credential still signs in to an intact workspace (SC-001, SC-002)
- [X] T045 [P] Read `docs/domain/rituals-tasks.md` and `docs/domain/compliance-safety.md` against what was actually implemented. No RPC, schema or job cadence changed, so no domain document should need an edit — but if implementation discovered behaviour that differs from those snapshots, record the difference in the drift register in `docs/domain/README.md` in this change set (Constitution XII, and the obligation the plan's Constitution Check accepted)
- [X] T046 [P] Review `docs/compliance/reviewer-notes.md` as a reader who has never seen this codebase: three credentials in one place, primary first, exactly one unmistakably named as the account to delete (FR-002, FR-008) — this is a human judgement no automated check can make, which is why it is a task rather than an assertion
- [X] T047 Re-read `backend/cmd/seed_demo.go` end to end as one linear script and delete any helper that is now called once and reads worse than its body inline; the file is expected to land near 700 lines and stays one file on purpose (Constitution V)
...
Recorded during `/speckit-implement` so the choices below can be reviewed rather than
rediscovered. Each is a decision taken where the task list or the design documents were
silent or wrong about this repository.

- **[ASSUMPTION: `make dev-up` and `make migrate` do not exist in this repository's
  Makefile, so T001 was satisfied with the tooling that does.** The stack was already up —
  `tech-office-backend-postgres-1` on `PG_PORT=15432` with every migration applied, and a
  backend answering `http://localhost:18080/healthz` — which is what `make check-postgres`
  and `make check-backend` actually assert. `backend/docker-compose.yml` has no Redis or
  MinIO service either; the quickstart's prerequisites section describes a stack this
  repository does not have. The quickstart was left as written rather than edited, because
  correcting it is a documentation change beyond this feature's scope.]

- **[ASSUMPTION: the seed now writes its work items before its conversation, reversing the
  order the plan implies.** A task captured from a chat message holds
  `(organization_id, source_message_id)` against `chat.message` under an `ON DELETE SET
  NULL` foreign key, and Postgres nulls *every* column of a composite key — including the
  `NOT NULL` `collaboration.task.organization_id`. Clearing the conversation while such a
  task exists therefore fails outright, which is exactly what happened on a local demo
  workspace where somebody had used the chat-to-task quick action: `seed-demo-org` exited
  non-zero on `clear demo messages`. Emptying the projects first removes the references and
  the seed survives having been used, which is the whole point of an idempotent fixture. The
  underlying constraint is recorded as **D75** in `docs/domain/README.md`; fixing it needs
  `ON DELETE SET NULL (source_channel_id, source_message_id)` and therefore a migration this
  feature does not carry.]

- **[ASSUMPTION: the seeded calendar entry now moves forward on every run.** T037 asked for
  every date to derive from `seededAt`; the pre-existing calendar insert used SQL `now()` and
  was guarded by `WHERE NOT EXISTS`, so on a second run it did not move at all and a demo
  seeded three weeks before review would show a site visit in the past. It is now written and
  then updated from `seededAt + 1 day`, which makes FR-020 true of the whole fixture rather
  than of the rows this feature added.]

- **[ASSUMPTION: `demoOwnerID` resolves the primary owner by email rather than by "whichever
  employee holds the owner role".** That question had one answer before this feature and has
  two after it. The previous `ORDER BY employee_id LIMIT 1` happens to return the primary
  owner, because UUIDv7 is time-ordered and the primary is created first — but it is the
  right answer by accident, and the account the seed reports as primary has to be the one the
  reviewer notes name.]

- **[ASSUMPTION: two pre-existing defects in `demo_seed_test.go` were fixed, because T040
  cannot pass around them.** The subdomain was `"seedtest" + uuid[:8]`, and the first eight
  hex characters of a UUIDv7 are the top bits of its millisecond timestamp, so two runs
  within about 65 seconds collided on one workspace and the second failed on "Reusing
  existing demo workspace" where it expected "Created". And the test's own `t.Cleanup` issued
  `SET LOCAL session_replication_role` outside a transaction, where it does nothing, so the
  delete that followed failed on foreign keys with its error discarded — the database held
  demo organisations from 5 September. Both files now use the full UUID hex and hand their
  organisation to `rememberOrg`, which is the package's working purge.]

- **[ASSUMPTION: T042's failure injection is performed by the automated test rather than by
  hand.** `TestDemoSeed/when a demo project is missing a workflow state the seed needs`
  deletes the `Overdue` state from the seeded `OPS` project and re-runs the command, which is
  the same injection the quickstart describes, on a workspace that is thrown away afterwards.
  Doing it a second time by hand against a scratch edit would test nothing further.]

- **[ASSUMPTION: three changes outside `backend/` and `docs/compliance/` were made, because
  T043 and T044 could not otherwise be performed at all.** Mobile email sign-in called
  `getProfile()` before storing the access token, and the RPC auth interceptor reads the
  token from storage — so on a genuinely signed-out device the profile call went out with no
  `Authorization` header, came back `Unauthenticated`, and the whole sign-in aborted with
  "Your session is no longer valid". It looked intermittent and iOS-specific only because a
  previous session's stored token masked it; it reproduced immediately on Android once the
  device was properly signed out. That is the exact state a store reviewer's fresh install
  is in, so none of the three credentials this feature exists to hand them could be used.
  `setAuthToken` now runs first in `signin.tsx`'s password and SSO paths and in
  `signup.tsx`, copying what the PIN paths in `(auth)/index.tsx` and `(auth)/set-pin.tsx`
  have always done. This closes **D57** in `docs/domain/README.md`, which is deleted rather
  than annotated, per that register's own rule.]

- **[ASSUMPTION: T040's failing suite was an environment gap, not a regression.** The first
  full run failed seven voice and native-call tests, all on
  `dial tcp [::1]:7880: connect: connection refused` — the LiveKit container was not
  running, because the stack had been started with `make infra-up`, which omits it, rather
  than `make voice-dev-infra-up`. After `docker compose up -d livekit` the whole package
  passes: `ok github.com/nvcnvn/tech-office/backend/integration 83.541s`, no failures.]

- **[ASSUMPTION: T043's iOS half is evidenced by screenshots rather than by Maestro
  assertions.** The Android pass ran end to end under Maestro. On the iOS simulator the same
  flow signs in and reaches every tab, but `assertVisible: "Open-up checks"` fails while the
  screenshot Maestro captures at that exact step shows the string twice — iOS's view
  hierarchy does not expose the nested card text as a matchable element. The scratch flow
  was deleted rather than committed, because the test-scenarios contract excludes Maestro
  from this feature's deliverables; it was a driver for the manual pass, not a new flow.]

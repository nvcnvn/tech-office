...
- [X] T001 Bring up the voice-capable dev stack per [quickstart.md](quickstart.md) §Prerequisites — `make infra-up`, `make voice-dev-infra-up`, `make check-servers` — confirming the test process and server agree on `PUBLIC_LIVEKIT_URL`
- [X] T002 Record a green baseline for the existing voice suite by running `cd backend && go test ./integration/ -run 'Voice' -count=1` and noting any pre-existing failure, so regressions introduced by this feature are distinguishable
...
- [X] T015 [P] [US2] Add the assertion to `frontend/apps/mobile/src/hooks/channel-voice-call-state.check.ts` that a `callEnded` event for a call this client never held leaves `error` in place (FR-005, FR-013)
...
- [X] T018 [P] [US2] Apply the same fix to `case "callEnded"` in the reducer in `frontend/apps/mobile/src/hooks/channel-voice-call-state.ts` — preserve `error` when there was no local call (FR-013, contract C4)
...
- [X] T020 [US2] Run `make test-frontend-one F=voice-communication` and `node --experimental-strip-types frontend/apps/mobile/src/hooks/channel-voice-call-state.check.ts` until both pass

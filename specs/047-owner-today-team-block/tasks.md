...
- [X] T044 [P] Add a "Team attention summary" subsection under Compliance and health in `docs/domain/rituals-tasks.md` describing the new RPC, its supervisory scope predicate, the two categories, the bounds and the fact that lateness is read from the stored state category rather than computed (Constitution XII).

...
- [ASSUMPTION: T013–T014 were executed without App Store Connect or Play Console
  credentials, so the question wording in `docs/compliance/age-rating-answers.md` is
  transcribed from the most recent form revision known at authoring time rather than read
  off the live consoles. Per the assumption recorded above, the document's
  "last checked against the live forms" date is set to **2026-05-31** — the date of that
  known form — rather than to today, and the document carries a prominent unverified
  banner instructing the next submitter to reconcile the question text against what is
  actually on screen and move the date forward. The **answers** are all verified against
  this repository and do not depend on the wording.]
- [ASSUMPTION: T007 and T031 expected `git diff main -- docs/compliance/permission-justifications.md`
  to be empty. That premise was wrong independently of this work: feature 053 (`764e18e`),
  which rewrote that file, is on this branch and **not** on `main`, so a diff against
  `main` necessarily shows all of 053. The meaningful comparison is `git diff HEAD`, which
  isolates feature 054's own change. Both recorded-absence rows FR-003 requires to be kept
  are present and untouched.]
- [ASSUMPTION: feature 054 makes exactly one edit to `permission-justifications.md`, which
  T007 expected to be a verification only. The gate implemented in T024 correctly failed on
  line 65, `"performs no background location updates, geofencing, or significant-change
  monitoring"` — a truthful *denial* sitting in the running prose of the
  `### NSLocationWhenInUseUsageDescription` justification, outside both of that document's
  declared recorded-absence sections. This is a real gap in the design rather than a bad
  match: R-5 chose declared sections over negation detection precisely because negation
  detection fails open, and the cost is that a denial in running prose has nowhere to live.
  The sentence was reworded to "takes no location readings while it is in the background,
  and performs no geofencing or significant-change monitoring" — identical meaning, and a
  genuine rewrite rather than a hyphenation trick that would have gamed the substring
  match and made the gate worthless. The alternative, declaring a third exempt section in
  that document, was rejected because it widens the exemption surface to a section whose
  purpose is to make live claims.]
- [ASSUMPTION: T020 corrects both cells of the Location row rather than only "where it is
  collected". The "Specific data" cell also read "captured once at check-in or on
  completing a task", which asserts collection at calendar check-in — the same false claim
  research R-4 disproves, in the same row. Correcting one cell and leaving the other would
  have left the row self-contradictory.]
- [ASSUMPTION: T032's cold-reader walkthrough was run against the repository only, not
  against the live consoles, for the same credential reason as T013–T014. The mechanical
  half is verified: 39 answer rows (23 App Store Connect, 16 Play Console/IARC), zero with
  an empty Evidence cell, zero whose Evidence merely restates the Answer, and both
  `### Resulting rating` sections filled in with their provisional markers (SC-004,
  SC-005). Eight Evidence cells were followed to their named file and line during
  authoring. The human half of SC-003 — somebody completing the form from the document —
  remains to be done at submission.]
- [ASSUMPTION: T033 ran `make test-mobile` in full. `check-store-manifest` passed as the
  target's first prerequisite and the `Makefile` is unchanged, so FR-019 and the gate half
  of SC-009 hold. The Maestro suite then aborted in its first flow on
  `Assert that id: xmark is visible` in `auth/dev-client-bootstrap.yaml` — which is **D73**
  in the drift register verbatim, pre-existing and unrelated. This feature changes no
  mobile UI, so the suite has nothing of its own to fail on, and no new drift is opened.]
...
- [X] T004 [US1] Remove the clause "Face ID (optional faster sign-in)" from the `### Permissions` paragraph of `docs/compliance/reviewer-notes.md` (line ~127), leaving exactly five permissions named — microphone, camera and photos, location, notifications — and removing rather than rewording the clause (FR-001, SC-002)
- [X] T005 [US1] Add a `### Not requested` section to `docs/compliance/reviewer-notes.md` immediately after `### Permissions`, and move the sentence "There is no background location." out of the permissions paragraph into it, phrased as what the app does not ask for and why (research R-5; this is the recorded-absence section the gate will declare for this file in T022). Depends on T004 — same paragraph
- [X] T006 [P] [US1] In `docs/compliance/data-collection-inventory.md` under `### Not collected` (line ~66 region), replace the sentence "Face ID and fingerprint sign-in are performed by the operating system; the app receives only success or failure and never the biometric itself" with a statement that the app performs no biometric authentication at all, describing no data flow for one (FR-002)
- [X] T007 [P] [US1] Verify `docs/compliance/permission-justifications.md` at HEAD contains no justification section for a permission the app does not request, and that both recorded-absence rows survive untouched — the `USE_BIOMETRIC`/`USE_FINGERPRINT` row in *Permissions deliberately blocked* and the `NSFaceIDUsageDescription` row in *Keys deliberately absent* (FR-003, research R-2). This task produces no edit; a non-empty `git diff main --` on this file at the end of the feature is a defect
- [X] T008 [US1] Sweep every `*.md` under `docs/compliance/` for the capability vocabulary in research R-6 and confirm each remaining occurrence is either a capability in the declared set or sits inside a recorded-absence section, fixing any occurrence that is neither (FR-004, SC-001). Depends on T004–T007
- [X] T009 [US1] Update the review-materials description in `docs/domain/compliance-safety.md` so its `## Permissions` and review-notes material match the corrected documents and no longer imply a biometric sign-in exists anywhere (FR-005, Constitution XII)
...
- [X] T028 [US3] Update the `## Store manifest` section of `docs/domain/compliance-safety.md` to describe the gate's new compliance-prose rule, its capability vocabulary, the recorded-absence exemption and the failure format, so the snapshot no longer implies the gate's only document cross-check is the justifications file (FR-005, Constitution XII). Depends on T024
...
- [X] T029 [P] Run quickstart §1's capability grep over `docs/compliance/` and confirm every hit falls inside a declared recorded-absence section and none in running prose (SC-001)
- [X] T030 [P] Run quickstart §2's one-copy check (`grep -rn "age rating" docs/compliance/ | grep -v age-rating-answers.md`) and confirm the only remaining hit is the pointer added in T019 (SC-006)
- [X] T031 [P] Confirm `git diff main -- docs/compliance/permission-justifications.md` is empty, proving FR-003 was a verification and not an accidental edit
- [X] T032 Walk quickstart §2's cold-reader procedure over `docs/compliance/age-rating-answers.md` with both questionnaires open: zero unanswered questions, zero escalations, and five Evidence cells spot-checked by following them (SC-003, SC-004)
- [X] T033 Run `make test-mobile` and confirm the gate passes and the suite proceeds with no step bypassing it, and that the `Makefile` is unchanged in the diff (FR-019, SC-009). Depends on T026
- [X] T034 Run quickstart §5's `git diff main -- docs/domain/compliance-safety.md docs/domain/README.md` and confirm the snapshot describes the corrected review materials and the new gate rule, the drift register gained the R-4 entry, and no entry describes behaviour this feature removed (FR-005, SC-009)

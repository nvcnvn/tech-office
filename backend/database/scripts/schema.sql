...
-- "voice" schema for voice communication lifecycle, invitations, artifacts, and voice-message metadata.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_iam_identity_org_email ON iam.identity(organization_id, email) WHERE email IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_identity_org_login_identifier ON iam.identity(organization_id, login_identifier) WHERE login_identifier IS NOT NULL;
...
-- =============================================================================
-- iam.credential: Org-scoped credentials for PIN and biometric authentication
-- Distributed on organization_id, colocated with public.organization
-- =============================================================================
...
-- =============================================================================
-- iam.account_lockout: Failed attempt tracking for PIN authentication
-- Distributed on organization_id, colocated with public.organization
-- =============================================================================
...
-- These employee foreign keys are declared after organization.employee is created
-- so this canonical schema can be applied top-to-bottom without forward references.
...
CREATE TABLE IF NOT EXISTS chat.message(
    ...
    message_kind text NOT NULL DEFAULT 'text', -- text, voice, system
    system_event_type text, -- voice_call_started, voice_call_ended, voice_call_missed, voice_call_cancelled
    ...
    CONSTRAINT fk_message_parent
        ...
        ON DELETE CASCADE,
    CONSTRAINT message_kind_valid CHECK (message_kind IN ('text', 'voice', 'system')),
    CONSTRAINT message_system_event_type_valid CHECK (
    ...
    CONSTRAINT message_system_event_consistency CHECK (
...
-- voice.call_session: Authoritative business record for one live or completed voice call.
...
SELECT create_distributed_table('voice.call_session', 'organization_id', colocate_with => 'public.organization');
...
-- voice.call_participant: Per-employee participation history and live reconciliation state.
...
SELECT create_distributed_table('voice.call_participant', 'organization_id', colocate_with => 'public.organization');
...
-- voice.call_invitation: Targeted invitation audit record for ongoing group calls.
...
SELECT create_distributed_table('voice.call_invitation', 'organization_id', colocate_with => 'public.organization');
...
-- voice.call_artifact: Recording and transcript artifact state for completed calls.
...
SELECT create_distributed_table('voice.call_artifact', 'organization_id', colocate_with => 'public.organization');
...
-- voice.voice_message: Voice-message upload, idempotency, and playback metadata.
...
SELECT create_distributed_table('voice.voice_message', 'organization_id', colocate_with => 'public.organization');
...
CREATE UNLOGGED TABLE IF NOT EXISTS notification.active_connection(
    ...
    CONSTRAINT fk_active_connection_employee
        ...
        REFERENCES organization.employee(organization_id, id)
        ON DELETE CASCADE,
    CONSTRAINT presence_status_valid CHECK (presence_status IN ('online', 'online_hidden', 'idle', 'offline', 'in_meeting')),
...
-- notification.active_listener: reference table for backend LISTEN topic registry
-- Tracks which backend instance currently owns a LISTEN connection and when it last heartbeated.
CREATE TABLE IF NOT EXISTS notification.active_listener (
    ...
    consumer_status text NOT NULL DEFAULT 'starting' CHECK (consumer_status IN ('starting', 'running', 'reconnecting', 'stopped')),
    consumer_last_active_at timestamptz,
...
-- notification.live_receipt: Client transport receipt separate from read acknowledgement
...
SELECT create_distributed_table('notification.live_receipt', 'organization_id', colocate_with => 'public.organization');
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_voice_invitation_notification') THEN
        ALTER TABLE voice.call_invitation
            ADD CONSTRAINT fk_voice_invitation_notification
            FOREIGN KEY (organization_id, notification_id)
            REFERENCES notification.notification(organization_id, id)
            ON DELETE RESTRICT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_voice_artifact_file') THEN
        ALTER TABLE voice.call_artifact
            ADD CONSTRAINT fk_voice_artifact_file
            FOREIGN KEY (organization_id, file_id)
            REFERENCES files.file_metadata(organization_id, id)
            ON DELETE RESTRICT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_voice_message_file') THEN
        ALTER TABLE voice.voice_message
            ADD CONSTRAINT fk_voice_message_file
            FOREIGN KEY (organization_id, file_id)
            REFERENCES files.file_metadata(organization_id, id)
            ON DELETE RESTRICT;
    END IF;
...
CREATE TABLE IF NOT EXISTS collaboration.project (
    ...
    -- Collaboration mode (UI display hint)
    collaboration_mode TEXT NOT NULL DEFAULT 'standard' CHECK (collaboration_mode IN ('standard', 'ritual', 'mixed')),
...
CREATE TABLE IF NOT EXISTS collaboration.project_state (
    ...
    category TEXT NOT NULL DEFAULT 'todo' CHECK (category IN ('todo', 'in_progress', 'done', 'cancelled', 'scheduled', 'submitted', 'verified', 'overdue', 'missed', 'skipped')),
    ...
    -- State type for swim lane separation in mixed projects
    state_type TEXT NOT NULL DEFAULT 'standard' CHECK (state_type IN ('standard', 'ritual')),
...
-- NOTE: fk_task_ritual_definition added after ritual_definition table is created (migration T008)
...
CREATE TABLE IF NOT EXISTS collaboration.saved_view (
    ...
    -- View type
    view_type TEXT NOT NULL CHECK (view_type IN ('board', 'list', 'gantt', 'calendar', 'today', 'health')),
...
-- =============================================================================
-- RITUAL DEFINITION — Recurring task template
-- =============================================================================
...
-- FK from task to ritual_definition (deferred from task table definition)
...
-- =============================================================================
-- RITUAL DEFINITION ASSIGNEE — Default assignees for ritual instances
-- =============================================================================
...
-- =============================================================================
-- RITUAL DEFINITION DEPARTMENT POOL — Dept-based pool assignment with strategy
-- =============================================================================
-- Each row defines a department as an assignee pool for a ritual definition.
-- During instance generation, the backend resolves one concrete employee_id
-- via 'round_robin' or 'least_assigned' strategy and updates the waterline.
...
CREATE TABLE IF NOT EXISTS collaboration.ritual_definition_department_pool (
    ...
    -- NULL = start from the beginning of the sorted member list.
...
-- =============================================================================
-- EVIDENCE REQUIREMENT — What evidence is needed per ritual definition
-- =============================================================================
...
-- =============================================================================
-- EVIDENCE SUBMISSION — Actual evidence submitted per task instance
-- =============================================================================

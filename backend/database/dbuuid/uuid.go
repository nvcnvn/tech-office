...
func (u NullUUID) MarshalJSON() ([]byte, error) {
	...
	return json.Marshal(uuid.UUID(u.UUID).String())
}
...
func UUIDToNullUUID(u UUID) NullUUID {
	return NullUUID{UUID: uuid.UUID(u), Valid: true}
}

func NullUUIDToUUID(nu NullUUID) UUID {
	return UUID(nu.UUID)
}

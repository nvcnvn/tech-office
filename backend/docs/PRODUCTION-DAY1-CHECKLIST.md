...
- [ ] **Backend SSO audiences**: `GOOGLE_CLIENT_IDS` is set to a comma-separated list of all OAuth Client IDs (web, iOS, Android) that the backend should accept in Google id_tokens. `APPLE_CLIENT_IDS` is set to the Apple Service ID(s) and app bundle ID(s). These are **runtime** env vars for the backend service. Leaving one unset no longer skips audience validation — it **disables that provider**: every token exchange and identity link for it is refused with `FAILED_PRECONDITION` / `SSO_PROVIDER_NOT_ENABLED`. So set both if you are shipping Google and Apple sign-in, and set neither if you are not. A value that parses to nothing (`" , "`) refuses to start.
...
- [ ] **`APP_ENV` is `production`** — which `deploy/stacks/core.yml` already declares, and which is also what an unset value means. In this profile the backend evaluates three checks before it opens a database connection or binds a port, and refuses to start if any fails, naming every failure at once: `JWT_PRIVATE_KEY_PATH` is set; `WEBAPP_URL` is an absolute, non-loopback address (it is also where the allowed browser origin comes from); and each of `GOOGLE_CLIENT_IDS` / `APPLE_CLIENT_IDS` either holds real client IDs or is left entirely unset. If the backend will not start, read the message — it names the setting, what the current state exposes, and what a correct value looks like.
- [ ] `CORS_ALLOWED_ORIGINS` is set only if browser clients are served from an origin other than `https://transformar.work`. A single-domain deployment leaves it empty; the origin derived from `WEBAPP_URL` is always allowed. A literal `*` refuses to start.
...
- [ ] `JWT_PRIVATE_KEY_PATH` points at a mounted production signing key. The backend will not start in the production profile without one — an ephemeral signer would sign everyone out on every restart and make two replicas reject each other's tokens.
- [ ] `WEBAPP_URL=https://transformar.work`. This value is used for IAM email links, and it is the origin browsers are allowed to call the API from. It must match the public web host exactly.
...
- [ ] `deploy/secrets/` holds a real `jwt-private.pem` — a placeholder or a missing file stops the backend from starting, not just from signing well. If push is in scope, it also holds a real `fcm.json` and `apns.p8`; those two are still soft, leaving push switched off with a startup log line rather than a refusal.

...
32 values, grouped: chat (`message`, `mention`, `reply`, `typing`, `reaction`), voice (4),
...
`ritual_instance_unassigned` is published by the ritual **shift resolution** sweep, described
in [rituals-tasks.md](rituals-tasks.md#shift-resolution--the-third-global-sweep-feature-042),
when an on-shift ritual instance reaches its scheduled date with nobody in the department
rostered. It goes to the project's owners and admins. It is deliberately not folded into
`ritual_instance_missed`: missed means somebody was asked to do the work and did not, and
this means nobody was ever asked — collapsing them would tell an owner to chase a person who
does not exist. It carries the same ordinary envelope as the two lateness types.

The same sweep reuses `task_assigned` for a newly bound assignee and `task_updated` for a
previous assignee whose instance a rota change moved off them. Neither needed a new type:
from the recipient's seat nothing distinguishes "a manager assigned this to you" from "the
rota did".

...
**Status date: 2026-09-04** · Branch at capture: `044-mobile-project-ritual-creation`
...
| D49 | Behaviour | [docs-knowledge.md](docs-knowledge.md#access-control) | `SearchDocuments` and the document tree listings (`ListRootDocuments`, `ListChildDocuments`) are **organization-scoped, not access-scoped**: they return every non-deleted document in the organization to any caller holding `docs.view`, with no `docs.document_access` predicate. Per-document access is enforced only from `GetDocument` downwards, so document titles — and search snippets, which are drawn from `content_text` — are visible far more widely than document content. This predates feature 043 and was found while building its procedure chooser, which is why that chooser can offer a document the manager cannot open and why the attachment is validated server-side with `CheckAccess` instead of trusting the picker. Closing it means adding an access predicate to the listing and search queries, which changes the docs tree for every user and is a change in its own right. |

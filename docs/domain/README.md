...
| D23 | Deferred | [voice.md](voice.md#native-call-presentation) | Spec 037's FR-021 (system recent-calls surface) is not implemented. Jetpack Telecom's unified call history and `isLogExcluded` require Android 16.1 (SDK 36.1), far above the epic's API 26 floor. FR-021 is a MAY; revisit when the 16.1 install base justifies it. |
| D25 | Behaviour | [voice.md](voice.md#native-call-presentation) | Mobile has no in-app mute control anywhere. `voiceClient.setMuted` is called only from `native-call.ts`, mirroring the OS call object; the channel's call banner and the global active-call bar report mute state but cannot change it. On a fallback tier-B device — one the OS does not ring for, so there is no system call UI — the user therefore cannot mute at all. |
| D26 | Behaviour | [voice.md](voice.md#native-call-presentation) | Declining a call that carries no invitation goes through `EndVoiceCall`, and `endOutcomeFor` records an unanswered call as `cancelled` — the outcome for the *caller* giving up. A decline and a caller cancel are therefore indistinguishable in the call record for that path. Both the system call UI (`native-call.ts`) and the in-app direct-call Decline share the rule, so the gap is in `internal/voice`, not the clients: `EndVoiceCall` has no way to say who walked away. || D15 | Cosmetic | — | The evidence-requirement editor shows the auto-approve switch for `photo` **and** `gps_checkin` (`AUTO_APPROVABLE_TYPES`), but labels it "Auto-approve via GPS check-in" in both cases. On a photo requirement the label describes a rule that does not apply to it. |

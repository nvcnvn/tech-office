...
- **Type** — `workspace_doc | task_description | project_brief`. Only `workspace_doc`
  appears in the workspace docs list; `task_description` documents belong to a task and are
  filtered out (see [rituals-tasks.md](rituals-tasks.md#tasks)).
...
Nothing document-specific. Two clarifications that read as drift:
